...
**710 unit tests plus 547 security-rules tests.** `pnpm verify` green.
...
| The patient | Today (windline, **the check-in itself from the chosen hour**, medication with "iets anders genomen?", activities, practices), check-in with the weekly akathisia screen and the hopelessness path, medication with "ask for a change" and "weer zelf beheren", the calendar and its suggestions tray, Overview and the diary archive, the printable report, the feed, settings (**including the light/dark switch**), the circle screens, `/dokter` to add a doctor by code **or by name**, `/plan` — **the six-step safety plan**, one page |
...
**the full rules matrix** (P8.3), **the error-handling sweep** (P8.4), the
paperwork as [docs/GDPR.md](docs/GDPR.md) (P8.5), **the Stanley–Brown safety
plan** — six sections on one page, replacing the single warning-signs list —
**the person's own crisis contacts**, and **supporter guidance on
`/following`**. All of
[docs/superpowers/plans/2026-08-06-safety-plan-supporters-and-hardening.md](docs/superpowers/plans/2026-08-06-safety-plan-supporters-and-hardening.md)
has landed.

**Still open: the family pilot (P8.6), and it is the only thing left.**
One thing P8.1 could not do here: a **real
screen-reader pass in Dutch** needs NVDA or VoiceOver driven by a person.
Accessible names, tab order and focus are verified; how it *sounds* is not. **Exit is v1 live, and then the PRD's own rule applies — stop, and two
...
**Icons only on the phone, and the word is still in the DOM.** Thomas asked
for the labels off the tab bar; they are visually hidden rather than deleted,
because a link with no accessible name is announced as "link" and nothing
else, which would leave the whole bar unusable with a screen reader. Above
768px they come back on screen, where there is room. If a sixth destination is
ever added, reconsider — five glyphs is near the limit of what is recognisable
without words, and concentration is the thing this app cannot assume.

The current tab is marked by full-strength text *and* a rule above it — never
colour alone (WCAG 1.4.1), the same reason there is no traffic-light coding
anywhere. The active state is `--text` rather than an accent: `--self` means
the person's own data and `--human` means somebody else was here, and which
tab you are on is neither.
...
| Date | Change |
|---|---|
| 2026-08-07 | **The safety plan, the person's own crisis contacts, supporter guidance, and P8.4 — the last of Phase 8 before the pilot.** Five phases, 33 commits, each reviewed before the next began. **The structural finding came before any code**: Stanley and Brown's steps 4 and 5 *are* the personalised crisis contacts, so those were one data model rather than two — `/crisis` reads the plan, and there is no second list to keep in sync. **What the reviews caught is the part worth recording, because every one was a thing the app said that was not true.** The check-in was the worst: `TodayCheckin` called `onSaved` synchronously, so a refused write's `.catch` always landed on an unmounted component and Today then rendered *"Je hebt vandaag ingevuld"* **and echoed the person's own diary line back at them** for a record that did not exist — that screen was worse after the sweep than before it, because the acknowledgement was affirmative. `CircleMember` and `Settings` never rolled back an optimistic permission toggle, and **narrowing is the dangerous direction**: instant and silent by design (D29), so a failed write left the switch reading OFF while the grant was still live and completions still posting — somebody believing they had closed access to Article 9 data. And `errorNotAllowed` — *"Vraag het aan wie het deelde"* — was written for the shared-data case and reused on seven screens where the person is writing **their own record**, implying they need somebody else's permission for their own data. **`offline` cannot be said everywhere.** It promises *wat je invult wordt bewaard en later verstuurd*, which is true of a queued `setDoc` and false of a `runTransaction`, and Firebase documents that transactions fail outright offline while plain writes simply do not settle. `redeemInvite` is the only transaction in the app; it gets its own wording, the same reason `authOffline` exists. **`reportError` logs the code and never the payload**, and that is now enforced rather than intended: a review defeated the first version by execution — a getter on `.code` returning a payload-laden string printed a diary line to a console that gets screenshotted into bug reports — so it matches the *shape* of a real Firebase code and falls closed on anything else, including a getter that throws. **Three guards were added and all three were proved by injection first**, which is this repo's rule and earned its keep twice over: `typeface.test.ts` scans every stylesheet for `--font-human` outside a five-entry allowlist, and found app-authored example prompts set in the serif **directly above the person's own entries** on the safety-plan screen — if that typeface is ever going to mean *a human wrote this*, it has to mean it there. The MDR boundary test in `supporting.test.ts` **missed this project's own canonical violating example** ("Jonas lijkt gespannen vandaag") because it required the words in the other order; it now catches the mechanism — a state claim about the supported person in the third person — in both languages, with the honest limit written down that a name with no pronoun still passes. And the matrix's own guard could not see `POST_SUBCOLLECTIONS`, four lines below the array it parses, so `reactions` and `comments` had real access rules and showed as silence — the exact failure the matrix exists to prevent. **Two plan defects found by building it:** `PLAN_EXAMPLE_KEYS` went dead-but-tested when the six-section screen dropped it (an intro says what goes in the box, an example shows how specific a useful answer is — different jobs, restored under `warning` only), and the tip rotation's `Math.abs` **mirrored around the 1970 epoch** rather than continuing, with the plan's own test passing either way. `sign`/`action` became `label`/`detail` now rather than never, which is safe **only** because prod is live and empty. 65 more unit tests (710), 12 more rules tests (547). |
| 2026-08-06 | **The safety plan's screen catches up to its model — six sections on one page.** `/plan` rebuilt from the single flat warning-signs list into Stanley & Brown's full six steps, each its own unit — heading, intro, its own entries, an always-open add form — because **somebody re-reads this on a bad day, and scrolling past five other steps beats finding them one screen at a time.** The MDR line stays exactly where it was, once, near the top. **Two things found while building it, not assumed away.** `readPlan` cast `data.section` straight to `PlanSection` with no check — harmless while nothing wrote a `section`, and this is the task that starts writing one on every entry, with `/plan/{entryId}` carrying no shape rule to catch a stray value; `toPlanSection` now defaults anything outside `PLAN_SECTIONS` to `warning`, so a bad value ages out instead of silently vanishing from every `entriesInSection` grouping. And an accessible-name collision the first pass missed: six sections asking near-identical questions need every field and button named by section, but an always-open add form and a row being edited can sit in the *same* section asking the *same* question, and in `warning` the field's own label and the section's own title are the identical string ("Wat je merkt") — naming every control `field – section` collided the add form with the row under edit. A test written to prove the fix caught it before it was ever committed; every row control is now named by what the person wrote on it, not only by which section it sits in. **Dropped the old example-prompt list**, on the assumption that six per-section intros now did what four warning-only examples had. That assumption went untested and was wrong: an intro says what kind of thing belongs in the box, an example shows how specific a good answer looks, and the one open-ended question of the six (`warning`) still needs both — caught in review and restored there the same day, `PLAN_EXAMPLE_KEYS` rendered again rather than merely exported. 13 more unit tests (645). |
