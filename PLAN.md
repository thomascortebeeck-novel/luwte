...
| D25 | **A prescription outlives the circle entry that authorised it, so the patient can take one back when the prescriber is gone.** Revoking a clinician otherwise froze every line they wrote: the patient fails the self-edit branch because `prescribedBy` is set, the clinician fails it because a revoked member is granted nothing, and `pendingChange` — the only move left — is addressed to somebody who can no longer read the document it lives on. Confirmed against the emulator before it was fixed. The release cannot carry a dose change (`onlyReleases()`), so it is not a way past the prescriber. What this leaves is narrower than "the patient may never disown a prescription", and the honest version is worth writing down: they cannot disown it **while that clinician is still theirs**, and can always revoke first — two writes, both visible. A stronger rule would be decoration, since the circle is theirs to change. Provenance is protected by `logOnlyGrows`, not by the freeze. |
| D26 | **Onboarding branches on `users.role`, and a supporter consents to confidentiality rather than to Article 9.** The role is inferred from a pending invite where one exists — the patient already chose it, which beats asking — and asked once otherwise. Asking is safe because choosing grants nothing: verification is an admin write to `clinicians/{uid}` (D20), so the answer routes screens and never access. The consent split is the substantive half: a supporter stores no health data of their own, so Article 9 consent from them would be meaningless and would obscure the obligation they do take on. The rules accept `essential` plus either `healthData` or `confidentiality` and refuse a record agreeing to neither. `checkinHour` is now written once, by the person who was asked, and `isDueForReminder` refuses a candidate without one instead of defaulting — otherwise the daily nudge simply moves to 9am for people who never wanted it. |
| D27 | **Verifying a clinician is a decision a person makes in the app, at `/admin`** (Thomas, 2026-08-05), rather than a shell script. Supersedes the "Doing it" half of D20; the rest of D20 stands. The out-of-band step is not removed, it moves down one level: `clinicians/{uid}` is written by an admin from the panel, and `admins/{uid}` is written only with the Admin SDK. An admin cannot make another admin, cannot enumerate the admins, and cannot sign a decision as somebody else — one test each. Applying grants nothing, and a decided request is kept rather than deleted, for the same reason a revoked circle member is. **The clamp lands with it:** a circle card naming somebody as a clinician requires `exists(/clinicians/$(memberUid))`, on create *and* update — the update half matters, because otherwise a supporter card could be promoted in a second write. Anyone at all may still be a supporter; verification gates the clinical role and nothing else. The cost Thomas accepted: a doctor must be verified before a patient can name them, so the pilot needs that one admin action first. |
| D28 | **A code connects; a name asks.** The two ways a patient reaches a doctor differ in who agreed to what, and the app should not flatten that. A connection code was *handed over* — that act is the clinician's consent, so entering it writes the circle card immediately. A name found in the directory was not, so that path issues an invite carrying `forUid`, and nothing exists until the clinician accepts it in their console. One field rather than a `careRequests` collection: two mechanisms to keep correct would be worse than one clause on the rule that already governs redemption. `list` widens to the addressee, which forces their query to filter on `forUid` and gives the console an inbox with no server. **There is no decline button** — the invite lapses after seven days and nobody is told, the same silence a declined activity gets; a button reporting a refusal would turn that silence into a message. Bearer link invites are unaffected: `forUid` is absent on them and defaults to null. |
